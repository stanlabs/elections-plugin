...
const DATA_FILE_PATH =
  ...
  window.ElectionsDashboardSettings &&
  typeof window.ElectionsDashboardSettings.dataUrl === "string" &&
  window.ElectionsDashboardSettings.dataUrl.trim() !== ""
    ? window.ElectionsDashboardSettings.dataUrl
    : "./election-data.json";
const DEFAULT_CANDIDATE_IMAGE_URL =
  "https://secure.gravatar.com/avatar/?s=64&d=mm&r=g";

const tabsEl = document.getElementById("jurisdictionTabs");
const resultsEl = document.getElementById("resultsContainer");
const roundTemplate = document.getElementById("roundTemplate");
const yearLogoEl = document.getElementById("yearLogo");
...
function renderYearLogo() {
  ...
  yearDigits.forEach((digit, index) => {
    ...
    part.className = `year-logo-digit ${index % 2 === 0 ? "year-logo-red" : "year-logo-blue"}`;
...
function buildCandidateCell(candidate, isWinner) {
  ...
  td.className = `candidate-cell${isWinner ? " candidate-cell--winner" : ""}`;
...
function renderSummaryTable(race, updatedAt) {
  ...
  wrap.className = "round-table-wrap";

  const metaBar = document.createElement("div");
  metaBar.className = "results-meta";
  const updatedText = document.createElement("span");
  updatedText.textContent = `Latest results from ${updatedAt}`;
  const certifiedText = document.createElement("span");
  certifiedText.textContent = "VOTE TOTALS CERTIFIED";
  metaBar.append(updatedText, certifiedText);
...
function renderWinnerBanner(race, jurisdiction) {
  ...
  labelRow.className = "winner-banner-label";

  const checkIcon = document.createElement("span");
  checkIcon.className = "winner-banner-check-icon";
  ...
  body.className = "winner-banner-body";

  const photo = document.createElement("img");
  photo.className = "winner-banner-photo";
  ...
  text.className = "winner-banner-text";
...
function renderRounds(race) {
  ...
  roundsWrap.className = "rounds-grid";
...
function createFullResultsContent(race) {
  ...
  full.className = "full-results";

  const title = document.createElement("h4");
  title.className = "full-results-title";
...
function renderRaceCard(race, updatedAt, jurisdiction) {
  ...
  description.className = "race-description";
  ...
  toggleArrow.className = "toggle-details-arrow";
  ...
  toggleButton.addEventListener("click", () => {
    const isOpen = fullResults.classList.toggle("open");
...
function renderJurisdictionResults() {
  ...
  heading.className = "jurisdiction-heading";
  ...
  jurisdictionGroup.elections.forEach((election) => {
    ...
    electionTitle.className = "election-title";
    electionTitle.textContent = election.title;

    const electionMeta = document.createElement("p");
    electionMeta.className = "election-meta";
    electionMeta.textContent = `Last Updated ${election.updatedAt}`;

    header.append(electionTitle, electionMeta);

    const raceStack = document.createElement("div");
    raceStack.className = "race-stack";
